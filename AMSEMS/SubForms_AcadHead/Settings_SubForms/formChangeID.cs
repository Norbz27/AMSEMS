...
namespace AMSEMS.SubForms_AcadHead
{
    public partial class formChangeID : KryptonForm
    {
        ...
        public formChangeID(formAccountSetting form)
        {
            ...
            cn.Open();
            cm = new SqlCommand("Select ID from tbl_acadHead_accounts where Unique_ID = '" + FormAcadHeadNavigatio.id + "'", cn);
            dr = cm.ExecuteReader();
            dr.Read();
        ...
        private void btnDone_Click(object sender, EventArgs e)
        {
            using (cn = new SqlConnection(SQL_Connection.connection))
            {
                ...
                {
                    cn.Open();
                    cm = new SqlCommand("Select ID from tbl_acadHead_accounts where Password = @CurrentPassword AND Unique_ID = @ConditionID", cn);
                    cm.Parameters.AddWithValue("@CurrentPassword", tbCurPass.Text);
                    cm.Parameters.AddWithValue("@ConditionID", FormAcadHeadNavigatio.id);
                    ...
                    using (SqlDataReader reader = cm.ExecuteReader())
                    {
                        ...
                        {
                            reader.Close();
                            cm = new SqlCommand("UPDATE tbl_acadHead_accounts SET ID = @NewValue WHERE Unique_ID = @ConditionValue", cn);
