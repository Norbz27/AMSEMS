    ...
    public partial class formAccountSetting : KryptonForm
    {
        ...
        public void loadData()
        {
            try
            {
                ...
                {
                    cn.Open();
                    cm = new SqlCommand("Select ID,Firstname,Middlename,Lastname from tbl_acadHead_accounts where Unique_ID = @id", cn);
                    cm.Parameters.AddWithValue("@id", FormAcadHeadNavigatio.id);
                    dr = cm.ExecuteReader();
        ...
        private void btnRemoveProf_Click(object sender, EventArgs e)
        {
            using (cn = new SqlConnection(SQL_Connection.connection))
            {
                ...
                {
                    cn.Open();
                    cm = new SqlCommand("UPDATE tbl_acadHead_accounts SET Profile_pic = DEFAULT WHERE Unique_ID = @ConditionValue", cn);
