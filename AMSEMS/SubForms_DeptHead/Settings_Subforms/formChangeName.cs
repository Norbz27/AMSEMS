...
namespace AMSEMS.SubForms_DeptHead
{
    public partial class formChangeName : KryptonForm
    {
        ...
        public formChangeName(formAccountSetting form)
        {
            ...
            cn.Open();
            cm = new SqlCommand("Select Firstname,Middlename,Lastname from tbl_deptHead_accounts where Unique_ID = '" + FormDeptHeadNavigation.id + "'", cn);
            dr = cm.ExecuteReader();
            dr.Read();
        ...
        private void btnDone_Click(object sender, EventArgs e)
        {
            using (cn = new SqlConnection(SQL_Connection.connection))
            {
                ...
                {
                    cn.Open();
                    cm = new SqlCommand("UPDATE tbl_deptHead_accounts SET Firstname = @NewValue1, Middlename = @NewValue2, Lastname = @NewValue3 WHERE Unique_ID = @ConditionValue", cn);
