...
namespace AMSEMS.SubForms_Teacher
{
    public partial class formAddSectionToSubject : KryptonForm
    {
        ...
        public void displaySectionStudents(string sectionname)
        {
            ...
            {
                cn.Open();
                string query = "SELECT s.ID AS id, UPPER(s.Lastname || ', ' || s.Firstname || ' ' || s.Middlename) AS Name FROM tbl_students_account s LEFT JOIN tbl_section sec ON s.Section = sec.Section_ID WHERE sec.Description = @Secdes AND sec.AcadLevel_ID = @acadlvl ORDER BY Name";
