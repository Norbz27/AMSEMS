...
namespace AMSEMS.SubForms_Teacher
{
    public partial class formChangeID : KryptonForm
    {
        ...
        public formChangeID(formAccountSetting form)
        {
            ...
            cn.Open();
            cm = new SqlCommand("Select ID from tbl_teacher_accounts where Unique_ID = '" + FormTeacherNavigation.id + "'", cn);
            dr = cm.ExecuteReader();
            dr.Read();
        ...
        private void btnDone_Click(object sender, EventArgs e)
        {
            using (SqlConnection cn = new SqlConnection(SQL_Connection.connection))
            {
                ...
                {
                    cn.Open();
                    using (SqlCommand cmCheckPassword = new SqlCommand("SELECT ID FROM tbl_teacher_accounts WHERE Password = @CurrentPassword AND Unique_ID = @ConditionID", cn))
                    {
                        cmCheckPassword.Parameters.AddWithValue("@CurrentPassword", tbCurPass.Text);
                        cmCheckPassword.Parameters.AddWithValue("@ConditionID", FormTeacherNavigation.id);
                        ...
                        using (SqlDataReader reader = cmCheckPassword.ExecuteReader())
                        {
                            ...
                            {
                                reader.Close();
                                using (SqlCommand cmUpdateID = new SqlCommand("UPDATE tbl_teacher_accounts SET ID = @NewValue WHERE Unique_ID = @ConditionValue", cn))
                                {
                                    cmUpdateID.Parameters.AddWithValue("@NewValue", tbSchoolID.Text);
                                    cmUpdateID.Parameters.AddWithValue("@ConditionValue", FormTeacherNavigation.id);
                                    ...
                                    form.loadData();
                                    FormTeacherNavigation.id = tbSchoolID.Text;
                                    this.Close();
                                }
                            }
                        }
