    ...
    public partial class formChangeName : KryptonForm
    {
        ...
        private void btnDone_Click(object sender, EventArgs e)
        {
            using (cn = new SqlConnection(SQL_Connection.connection))
            {
                ...
                {
                    cn.Open();
                    cm = new SqlCommand("UPDATE tbl_teacher_accounts SET Firstname = @NewValue1, Middlename = @NewValue2, Lastname = @NewValue3 WHERE Unique_ID = @ConditionValue", cn);
                    ...
                    var result = pusher.TriggerAsync("amsems", FormTeacherNavigation.id, new { message = "new notification" });
                }
            }
