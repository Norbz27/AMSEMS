...
namespace AMSEMS.SubForms_SAO
{
    public partial class formChangeName : KryptonForm
    {
        ...
        public formChangeName(formAccountSetting form)
        {
            ...
            cn.Open();
            cm = new SqlCommand("Select Firstname,Middlename,Lastname from tbl_sao_accounts where Unique_ID = '" + FormSAONavigation.id + "'", cn);
            dr = cm.ExecuteReader();
            dr.Read();
        ...
        private void btnDone_Click(object sender, EventArgs e)
        {
            using (cn = new SqlConnection(SQL_Connection.connection))
            {
                ...
                {
                    cn.Open();
                    cm = new SqlCommand("UPDATE tbl_sao_accounts SET Firstname = @NewValue1, Middlename = @NewValue2, Lastname = @NewValue3 WHERE Unique_ID = @ConditionValue", cn);
                    ...
                    var result = pusher.TriggerAsync("amsems", FormSAONavigation.id, new { message = "new notification" });
                }
            }
