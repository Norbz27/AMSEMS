...
{
    public partial class formArchived_Accounts_Teachers : KryptonForm
    {
        SqlConnection cn;
        SqlDataAdapter ad;
        SqlCommand cm;
        ...
        private void backgroundWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            displayTable("Select ID,Firstname,Lastname,Password,d.Description as dDes, FORMAT(Archived_Date, 'yyyy-MM-dd') AS archdate from tbl_archived_teacher_accounts as te left join tbl_Departments as d on te.Department = d.Department_ID");
        ...
        private bool DeleteStudentRecord(int ID)
        {
            using (SqlConnection connection = new SqlConnection(SQL_Connection.connection))
            {
                ...
                {
                    connection.Open();
                    string deleteQuery = "DELETE FROM tbl_archived_teacher_accounts WHERE ID = @ID";
        ...
        private bool Unarchive(int ID)
        {
            using (SqlConnection cn = new SqlConnection(SQL_Connection.connection))
            {
                try
                {
                    ...
                    string checkExistingQuery = "SELECT COUNT(*) FROM tbl_teacher_accounts WHERE ID = @ID";
                    using (SqlCommand checkExistingCommand = new SqlCommand(checkExistingQuery, cn))
                    {
                        ...
                        if (existingRecordCount == 0)
                        {
                            ...
                            string insertQuery = "SET IDENTITY_INSERT tbl_teacher_accounts ON; " +
                                "INSERT INTO tbl_teacher_accounts (Unique_ID,ID,Firstname,Lastname,Middlename,Password,Profile_pic,Department,Role,Status,DateTime) SELECT Unique_ID,ID,Firstname,Lastname,Middlename,Password,Profile_pic,Department,Role,Status,DateTime FROM tbl_archived_teacher_accounts WHERE ID = @ID; " +
        ...
        private async void cbDep_SelectedIndexChanged(object sender, EventArgs e)
        {
            UseWaitCursor = true;
            ...
            if (!string.IsNullOrEmpty(filtertbl))
            {
                ...
                string query = "Select ID,Firstname,Lastname,Password,d.Description as dDes, st.Description as stDes from tbl_archived_teacher_accounts as te left join tbl_Departments as d on te.Department = d.Department_ID left join tbl_status as st on te.Status = st.Status_ID " +
        ...
        private void formArchived_Accounts_Teachers_FormClosed(object sender, FormClosedEventArgs e)
        {
            form.loadData();
        }
