...
namespace AMSEMS.SubForms_Admin
{
    public partial class formAcctounts_DeptHead : Form
    {
        ...
        private void ExportToPDF(DataGridView dataGridView, string filePath)
        {
            ...
            Paragraph titleParagraph = new Paragraph("List of Department Heads:", headerFont);
        ...
        private bool AddtoArchive(string teacherID)
        {
            using (SqlConnection cn = new SqlConnection(SQL_Connection.connection))
            {
                try
                {
                    ...
                    string checkExistingQuery = "SELECT COUNT(*) FROM tbl_archived_deptHead_accounts WHERE ID = @ID";
                    using (SqlCommand checkExistingCommand = new SqlCommand(checkExistingQuery, cn))
                    {
                        ...
                        if (existingRecordCount == 0)
                        {
                            ...
                            string updateStatusQuery = "UPDATE tbl_deptHead_accounts SET Status = 2 WHERE ID = @ID";
            ...
            try
            {
                using (SaveFileDialog saveFileDialog = new SaveFileDialog())
                {
                    ...
                    if (saveFileDialog.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        string query = "SELECT ID, UPPER(Firstname) as Firstname, UPPER(Middlename) as Middlename, UPPER(Lastname) as Lastname FROM tbl_deptHead_accounts te left join tbl_Departments as d on te.Department = d.Department_ID left join tbl_status as st on te.Status = st.Status_ID WHERE (@DepartmentDescription IS NULL OR d.Description = @DepartmentDescription) AND (@StatusDescription = 'All' OR st.Description = @StatusDescription) ORDER BY d.Description, Lastname";
        ...
        private void formAcctounts_DeptHead_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void formAcctounts_DeptHead_KeyDown(object sender, KeyEventArgs e)
        {
